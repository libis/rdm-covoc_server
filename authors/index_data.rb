...
class CSV
  module ProgressBar
    ...
    def progressbar
      return @progress_bar ||= TTY::ProgressBar.new("Reading data (:percent) [ETA: :eta]", total: @io.size)
    end
...
File.open(ARGV[0], 'rt') do |file|
  ...
  bar = TTY::ProgressBar.new("Formatting (:percent) [ETA: :eta]", total: a.size)
  ...
  bar.complete
  bar = TTY::ProgressBar.new("Indexing (:percent) [ETA: :eta]", total: a.size)
