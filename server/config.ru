...
class App < Roda
  ...
  PUBLICATIONS = JSON.load(File.read(ENV['PUB_DATA'] || 'publications.json'))
  ...
  route do |r|
    ...
    r.get 'publications' do
      ...
      from = typecast_params.int('from') || 0
      per_page = typecast_params.int('per_page') || 10

      res = {
        'numFound' => PUBLICATIONS.size,
        'start' => from,
        'numFoundExact' => true,
        'docs' => PUBLICATIONS
      }
